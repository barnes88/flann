...
struct smallerWithTolerance {
  float tol;
  bool operator()(float a, float b) { return a < (b - tol); }
};

/** @brief Compare the distances for match accuracies
 * This is more precise: e.g. when you ask for the top 10 neighbors and they all
 * get the same distance, you might have 100 other neighbors that are at the
 * same distance and simply matching the indices is not the way to go
 ...
 */
template <typename T>
float computePrecisionDiscrete(const flann::Matrix<T>& gt_dists,
                               const flann::Matrix<T>& dists, float tol) {
  int count = 0;
  ...
  for (size_t i = 0; i < gt_dists.rows; ++i) {
    ...
    typename std::vector<T>::iterator end = std::set_intersection(
        gt_sorted_dists.begin(), gt_sorted_dists.end(), sorted_dists.begin(),
        sorted_dists.end(), intersection.begin(), swt);
...
class FLANNTestFixture : public ::testing::Test {
 protected:
  ...
  double stop_timer() { return double(clock() - start_time_) / CLOCKS_PER_SEC; }
};

class Flann_sift10k : public FLANNTestFixture {
 protected:
...
class Flann_3D_Random_Cloud : public FLANNTestFixture {
 protected:
  ...
  void SetUp() {
    ...
    dists = flann::Matrix<float>(new float[query.rows * max_nn], query.rows,
                                 max_nn);
    gt_dists = flann::Matrix<float>(new float[query.rows * max_nn], query.rows,
                                    max_nn);
    indices =
        flann::Matrix<int>(new int[query.rows * max_nn], query.rows, max_nn);
    gt_indices =
        flann::Matrix<int>(new int[query.rows * max_nn], query.rows, max_nn);
    ...
    // 		{
    // 			std::cout<<gt_indices[i][0]<<"
    // "<<gt_dists[i][0]<<std::endl;
    // 		}
    printf("done (%g seconds)\n", stop_timer());
  }
...
// 	flann::Matrix<float> data_device_matrix(
// (float*)thrust::raw_pointer_cast(&data_device[0]),data.rows,3,4*4);
// 	flann::Matrix<float> query_device_matrix(
// (float*)thrust::raw_pointer_cast(&query_device[0]),data.rows,3,4*4);
...
// 	flann::Matrix<int> indices_device_matrix(
// (int*)thrust::raw_pointer_cast(&indices_device[0]),query.rows,4);
// 	flann::Matrix<float> dists_device_matrix(
// (float*)thrust::raw_pointer_cast(&dists_device[0]),query.rows,4);
...
//     index.knnSearch(query_device_matrix, indices_device_matrix,
//     dists_device_matrix, 4, sp ); printf("done (%g seconds)\n",
//     stop_timer());
...
// ); 	thrust::copy( indices_device.begin(), indices_device.end(),
// indices_host.ptr() );
...
// {
//     flann::Index<L2_Simple<float> > index(data,
//     flann::KDTreeCuda3dIndexParams()); start_timer("Building kd-tree
//     index..."); index.buildIndex(); printf("done (%g seconds)\n",
//     stop_timer());
// 	float r = 0.1;
...
// 			for( int k=0; k<3; k++ )
// 				dist +=
// (query[i][k]-data[j][k])*(query[i][k]-data[j][k]); 			if( dist
// < r*r )
// 			{
// 				EXPECT_TRUE( std::find( indices[i].begin(),
// indices[i].end(), j )!=indices[i].end() );
// 			}
// 			else
// 			{
// 				EXPECT_TRUE( std::find( indices[i].begin(),
...
// 		for( int j=0; j<data.rows; j++ )
// 		{
// 			// for each pair of query and data points: either the
// distance between them
// 			// is smaller than r AND the point is in the result set,
// or
...
// 	flann::Matrix<int> indices( new int[max_neighbors*query.rows],
// query.rows, max_neighbors ); 	flann::Matrix<float> dists( new
// float[max_neighbors*query.rows], query.rows, max_neighbors );
...
// < r*r )
// 			{
// 				EXPECT_TRUE( std::find( indices[i],
// indices[i]+max_neighbors, j )!=indices[i]+max_neighbors );
// 			}
// 			else
// 			{
// 				EXPECT_TRUE( std::find( indices[i],
