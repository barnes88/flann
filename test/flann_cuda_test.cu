...
TEST_F(Flann_3D_Random_Cloud, TestRadiusSearchVector) {
  ...
  for (int i = 0; i < query.rows; i++) {
    for (int j = 0; j < data.rows; j++) {
      ...
      if (dist < r * r) {
        EXPECT_TRUE(std::find(indices[i].begin(), indices[i].end(), j) !=
                    indices[i].end());
      } else {
        EXPECT_TRUE(std::find(indices[i].begin(), indices[i].end(), j) ==
                    indices[i].end());
  ...
  for (int i = 0; i < query.rows; i++) {
    for (int j = 0; j < data.rows; j++) {
      // for each pair of query and data points: either the distance between
      // them is smaller than r AND the point is in the result set, or the
      // distance is larger and it is not.
      float dist = 0;
      for (int k = 0; k < 3; k++)
        dist += (query[i][k] - data[j][k]) * (query[i][k] - data[j][k]);
      if (dist < r * r) {
...
TEST_F(Flann_3D_Random_Cloud, TestRadiusSearchMatrix) {
  ...
  flann::Matrix<int> indices(new int[max_neighbors * query.rows], query.rows,
                             max_neighbors);
  flann::Matrix<float> dists(new float[max_neighbors * query.rows], query.rows,
                             max_neighbors);
  ...
  for (int i = 0; i < query.rows; i++) {
    for (int j = 0; j < data.rows; j++) {
      ...
      for (int k = 0; k < 3; k++)
        dist += (query[i][k] - data[j][k]) * (query[i][k] - data[j][k]);
      if (dist < r * r) {
        EXPECT_TRUE(std::find(indices[i], indices[i] + max_neighbors, j) !=
                    indices[i] + max_neighbors);
      } else {
        EXPECT_TRUE(std::find(indices[i], indices[i] + max_neighbors, j) ==
                    indices[i] + max_neighbors);
      }
